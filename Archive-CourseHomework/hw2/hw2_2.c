...
int randomSelectNextMove(const int ibug, const int jbug, const int n, const int m)
{
	...
	{
		case MIDDLE	:
			return nextMove[rand() % 8];

		case LEFT_WALL :
			return nextMove[rand() % 5 + 7];
		case RIGHT_WALL :
			return nextMove[rand() % 5 + 3];
		case UPPER_WALL :
			return nextMove[rand() % 5 + 1];
		case LOWER_WALL :
			return nextMove[rand() % 5 + 5];

		case UPPERLEFT_CORNER :
			return nextMove[rand() % 3 + 1];
		case LOWERLEFT_CORNER :
			return nextMove[rand() % 3 + 7];
		case UPPERRIGHT_CORNER :
			return nextMove[rand() % 3 + 3];
		case LOWERRIGHT_CORNER :
			return nextMove[rand() % 3 + 5];
